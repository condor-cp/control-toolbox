...
public:
    ...
    using box_constraint_t = EstimatorStateBoxConstraint<STATE_DIM, SCALAR>;
    ...
    EstimatorBase(std::shared_ptr<SystemModelBase<STATE_DIM, CONTROL_DIM, SCALAR>> f,
        ...
        const box_constraint_t& box_constraint = box_constraint_t())
        : f_(f), h_(h), x_est_(x0), box_constraint_(box_constraint)
    {
    }

    //! Copy constructor.
    EstimatorBase(const EstimatorBase& arg)
        : f_(arg.f_->clone()), h_(arg.h_->clone()), x_est_(arg.x_est_), box_constraint_(arg.box_constraint_)
    {
    }
...
protected:
    ...
    box_constraint_t box_constraint_;
};
