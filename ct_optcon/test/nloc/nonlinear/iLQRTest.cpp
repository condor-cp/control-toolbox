...
TEST(ILQRTestA, InstancesComparison)
{
    try
    {
        ...
        NLOptConSettings<double> ilqr_settings;
        ...
        ilqr_settings.discretization = NLOptConSettings<double>::APPROXIMATION::FORWARD_EULER;
        ilqr_settings.nlocp_algorithm = NLOptConSettings<double>::NLOCP_ALGORITHM::ILQR;
        ilqr_settings.lqocp_solver = NLOptConSettings<double>::LQOCP_SOLVER::GNRICCATI_SOLVER;
        ...
        NLOptConSettings<double> ilqr_settings_mp = ilqr_settings;
        ...
        for (size_t i = 0; i < nTests; i++)
        {
            ...
            while (foundBetter)
            {
                ...
                for (size_t j = 0; j < xRollout.size() - 1; j++)
                {
                    ...
                    if (ilqr_settings.discretization == NLOptConSettings<double>::APPROXIMATION::FORWARD_EULER)
                    {
                        A_analytic =
                    ...
                    else if (ilqr_settings.discretization == NLOptConSettings<double>::APPROXIMATION::BACKWARD_EULER)
                    {
                        state_matrix_t aNew =
                            ilqr_settings.dt * analyticLinearSystem->getDerivativeState(xRollout[j], uRollout[j], 0);
                    ...
                    else if (ilqr_settings.discretization == NLOptConSettings<double>::APPROXIMATION::TUSTIN)
                    {
